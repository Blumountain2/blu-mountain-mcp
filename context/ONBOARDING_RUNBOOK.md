...
One install. Formerly two (the Public App, then a separate MCP Auth App
install for `mcp.hubspot.com`) — the second step is gone entirely as of
the REST pivot (`openspec/changes/hubspot-rest-api-pivot`, cutover
2026-09-02): `HubSpotDataPullClient` reads HubSpot's plain REST API
(`api.hubapi.com`) directly with the Public App's own token, so there's no
second credential to install.
...
and its manifest (`hubspot-app/src/app/app-hsmeta.json`) must declare every
scope in `HUBSPOT_SCOPES`/`HUBSPOT_OPTIONAL_SCOPES` — HubSpot validates a
request against the app's own registered `requiredScopes`/`optionalScopes`
regardless of which request param (`scope` vs `optional_scope`) a given
install sends a scope under, so the two must always be edited and deployed
(`hs project upload`) together.
...
1. Send the client's portal admin (whoever can approve app installs on that
  HubSpot portal) to `GET /install` on the service. This redirects them to
   HubSpot's own authorization screen, listing the scopes in `HUBSPOT_SCOPES`
   (required — the whole install fails if the portal can't grant one) plus
   whichever of `HUBSPOT_OPTIONAL_SCOPES` the portal's own subscription tier
   supports (e.g. `marketing.campaigns.read` needs Marketing Hub
   Professional+; a portal without it just doesn't get that scope, the
   install still succeeds) — all read-only, per this project's
...
4. A successful callback shows the portal admin a plain success page
  (`auth/pages.py`, not raw JSON — the real caller here is a browser, not a
   script) confirming the connection is complete, and is recorded in
   `audit_log` as `hubspot_install_completed`. The `hub_id` shown on that
   page is what every later step — the scheduled sync, the live session's
   tenant selection — refers to this client by.
5. Confirm the install is live: the next scheduled sync cycle
   ...
   to a separate list by hand.

**Re-running `/install` for an already-installed portal** (e.g. after
adding a new scope to `HUBSPOT_SCOPES`/`HUBSPOT_OPTIONAL_SCOPES`) requests
a fresh token with the current scope set — an existing vaulted token does
not retroactively gain new scopes on its own; confirm the new grant took
via HubSpot's own token introspection (`GET
https://api.hubapi.com/oauth/v1/access-tokens/{token}`) if in doubt.
...
- HubSpot's own consent screen shows "Authorization failed because your
account lacks access to the required scopes [X]" instead of a consent
screen at all: `X` is listed under `HUBSPOT_SCOPES` (required) but this
portal's subscription tier can't grant it. Confirmed live twice this
session: `marketing.campaigns.read` needs Marketing Hub Professional+, and
this exact error still appeared even after moving the scope to
`HUBSPOT_OPTIONAL_SCOPES` — because the Public App's own manifest
(`hubspot-app/src/app/app-hsmeta.json`) still listed it under
`requiredScopes` and hadn't been redeployed. HubSpot validates against the
app's own registered manifest, not which request param a given install
happens to send a scope under — the fix is moving the scope to
`optionalScopes` in the manifest too, deploying (`hs project upload`), and
only then retrying `/install`.
