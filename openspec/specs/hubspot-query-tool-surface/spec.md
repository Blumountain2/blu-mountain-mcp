# hubspot-query-tool-surface Specification

## Purpose
The live interactive session's HubSpot query tools — grouped by category (CRM records, engagement records, marketing content, users) rather than one generic tool or one per object type, each scopable to specific fields — as the interface a connecting staff session actually calls.
## Requirements
