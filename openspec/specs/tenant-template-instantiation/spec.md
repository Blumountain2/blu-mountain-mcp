# tenant-template-instantiation Specification

## Purpose
Producing a named, tenant-scoped onboarding profile — which of a tenant's real, populated HubSpot fields are confirmed relevant, informed by its known vertical framework's guidance and finalized only after human review — as the runtime parameterization input a tenant's own analysis eventually reads, never a copy of the shared framework or skill content itself.
## Requirements
