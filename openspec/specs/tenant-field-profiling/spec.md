# tenant-field-profiling Specification

## Purpose
Discovering which of a tenant's real HubSpot fields are actually populated and in active use, cross-referenced against a known vertical framework's own default-trust guidance where available — reusing the existing, already-allowlisted pull path, never a new HubSpot access mechanism.
## Requirements
