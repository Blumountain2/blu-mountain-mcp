# client-vertical-agent-classes Specification

## Purpose
Real, separate, version-controlled agent code per business vertical and per client — a shared base class holding only the tool-calling loop mechanics, one subclass per vertical, and one subclass per client declaring which HubSpot fields (standard or custom-object) it looks at — replacing a prior database-row-driven configuration model.
## Requirements
