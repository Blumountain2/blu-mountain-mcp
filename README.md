...
HubSpot OAuth, the token vault, the data pull, Airtable staging, Sybill ingestion, and the live session are all built and unit-tested — 144 tests passing against a real Postgres instance, re-run automatically on every push/PR to `dev`/`main` (`.github/workflows/ci.yml`). Real HubSpot Public App and MCP Auth App credentials are registered and both installs have been completed end to end against a real test portal. Blocked on one remaining pending external credential before real end-to-end use: a real Google Cloud OAuth client for the live session. Ask a teammate for the current phase plan and project background if you need it.
