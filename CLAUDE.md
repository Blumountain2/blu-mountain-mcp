...
- Direct Google/Microsoft (separate surface, `session/staff_auth.py`, kept but unhooked from `main.py`): needs `GOOGLE_CLIENT_ID`, `GOOGLE_HOSTED_DOMAIN`, `MICROSOFT_CLIENT_ID`, `MICROSOFT_TENANT_ID` (read via `os.getenv`, not `config.settings`) — no client secret for either provider, this path only verifies already-issued JWTs (audience + provider JWKS), it never exchanges a code. **Deliberately absent from `.env.example` and `docker-compose.yml`'s `environment:` block** as of 2026-08-19, since nothing running today reads them and their presence there only produced an unset-variable warning on every `docker compose` invocation; add them back to both if `staff_auth.py` is ever wired back in.

FastMCP OAuth Proxy variables (`FASTMCP_GOOGLE_CLIENT_ID`, `FASTMCP_GOOGLE_CLIENT_SECRET`, `FASTMCP_BASE_URL`, `FASTMCP_OAUTH_REDIRECT_URI`, `FASTMCP_ALLOWED_GOOGLE_DOMAINS`, `FASTMCP_ACCESS_TOKEN_TTL_MINUTES`) are in `.env.example`, and the real Google Cloud OAuth client they hold values for is registered and confirmed working end to end (see the Current build state section above). WorkOS has been fully removed — no `WORKOS_*` variables remain anywhere in this project.
